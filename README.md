...
> **Status: Phase 1+2/3 bridge.** The full web app, API, database, and playback
> service are runnable today. Playback uses **ffmpeg** for DeckLink/SDI and
> audio paths plus **mpv `--vo=drm`** for GPU/KMS connectors, consuming the
> selected video/audio outputs, tone-mapping profile, and idle screen settings;
...
| `HTM_MEDIA_ROOT` | Path where backend scans that media inside its container |
| `HTM_PLAYBACK_MEDIA_ROOT` | Path handed to playback and mounted into the playback container |
...
| `HTM_PLAYBACK_URL` | Playback control service |
| `HTM_PLAYBACK_DRIVER` | Playback engine, normally `ffmpeg` |
...
The backend scans media through `HTM_MEDIA_ROOT`; playback receives paths under
`HTM_PLAYBACK_MEDIA_ROOT`. The installer sets `HTM_PLAYBACK_MEDIA_ROOT` to the
host media folder you choose, so a setup pointed at `/home/htm` sends playback
`/home/htm/...` paths instead of backend-only `/mnt/media/...` paths.
...
### Playback runner (ffmpeg + mpv)

The playback service defaults to `HTM_PLAYBACK_DRIVER=ffmpeg`. The runner uses
the Settings tab payload on every `load`/`configure`:
...
playback/       Playback control service (ffmpeg, mpv, output discovery)
